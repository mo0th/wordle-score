...
const Button: Component<ComponentProps<'button'> & { block?: boolean }> = _props => {
  const [local, delegated] = splitProps(mergeProps({ block: false, type: 'button' }, _props), [
    'block',
    'class',
  ])
  ...
  return (
    <button
      class={cx(
        'focus-outline rounded px-2 py-1 transition-colors',
